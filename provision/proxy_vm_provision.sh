...
mkdir /etc/ssl/certs/sslcert

#копіювання сертифікатів - їх ліпше згенерувати знов,бо зараз там мої данні
cp /vagrant/distro/sslnginx-selfsigned.crt /etc/ssl/certs/sslcert
cp /vagrant/distro/sslnginx-selfsigned.key /etc/ssl/certs/sslcert
cp /vagrant/distro/dhparam.pem /etc/ssl/certs/sslcert


#зміна прав на сертифікат і ключ
chmod 600 /etc/ssl/certs/sslcert/nginx-selfsigned.key
chmod 644 /etc/ssl/certs/sslcert/nginx-selfsigned.crt
